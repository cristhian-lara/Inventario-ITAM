export interface IMailerService {
    ...
    sendReturnEmail(to: string, assignmentId: string, token: string, documentPath?: string, overrideUrl?: string): Promise<void>;
    sendMaintenanceSignatureEmail(to: string, maintenanceId: string, token: string, documentPath?: string): Promise<void>;
    sendFinalPdfEmail(to: string, documentPath: string): Promise<void>;
}
